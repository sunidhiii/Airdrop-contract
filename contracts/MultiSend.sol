...
contract MultiSend is Ownable, ReentrancyGuard {
    ...
    IERC20[] public tokenList;      // Multi-token purpose
    ...
    mapping(address => uint256) public tokenBalances;    // balance of each token in smart contract
    mapping (address => bool) public tokenExistence;

    constructor() public {}

    // Load amount in the contract
    function loadToken(address _token, uint256 _amount) external onlyOwner {
        if(!tokenExistence[_token]) {
            tokenList.push(IERC20(_token));
            tokenExistence[_token] = true;
        }

        IERC20(_token).transferFrom(msg.sender, address(this), _amount);    // depositing the token amount in the smart-contract
        tokenBalances[_token] = tokenBalances[_token].add(_amount);     // update balances
    }
 
    // Unload amount in the contract
    function withdrawToken(address _token, uint256 _amount) external onlyOwner nonReentrant {
        require(tokenExistence[_token], "Withdrawl from non existent token");
        require(tokenBalances[_token] >= _amount, "Not enough balance");

        IERC20(_token).transfer(msg.sender, _amount);       // removing the token amount in the smart-contract
        tokenBalances[_token] = tokenBalances[_token].sub(_amount);
    }

    // airdrop
    function distributeToken(address _token, uint _amount) external onlyOwner nonReentrant {
        require(tokenBalances[_token] >= _amount, "The value is not sufficient");
        require(nomineeList.length > 0, "No whitelisted addresses");

        tokenBalances[_token] = tokenBalances[_token].sub(_amount);     // update balances
        uint256 amountPerNominee = _amount.div(nomineeList.length);     // partPerNominee

        for (uint i=0; i < nomineeList.length; i++) {
            _distributeToken(_token, nomineeList[i], amountPerNominee);
        }
    }
